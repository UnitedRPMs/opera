...
if [ "$task" = 64 ]; then
libdir=lib64
package=$(curl -s http://get.geo.opera.com.global.prod.fastly.net/pub/opera/desktop/$ver/linux/ | grep 'amd64' | awk -F '<a href="' '{print $2}' | awk -F '">' '{print $1}' | grep .deb) 
elif [ "$task" = 32 ]; then
libdir=lib
package=$(curl -s http://get.geo.opera.com.global.prod.fastly.net/pub/opera/desktop/$ver/linux/ | grep 'i386' | awk -F '<a href="' '{print $2}' | awk -F '">' '{print $1}' | grep .deb)
else
libdir=lib64
package=$(curl -s http://get.geo.opera.com.global.prod.fastly.net/pub/opera/desktop/$ver/linux/ | grep 'amd64' | awk -F '<a href="' '{print $2}' | awk -F '">' '{print $1}' | grep .deb)
fi 
...
wget -c -P $HOME/rpmbuild/SOURCES/ $Source
